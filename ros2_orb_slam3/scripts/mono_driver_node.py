...
class MonoDriver(Node):
    def __init__(self, node_name = "mono_py_node"):
        ...
        # Subscriber to the RealSense camera node
    ...
    def subscribe_to_camera(self):
        """Subscribe to the RealSense camera node to receive image frames."""
    ...
    def camera_callback(self, msg):
        """Callback function for handling received camera frames."""
        try:
            # Convert ROS Image message to OpenCV image
            cv_image = self.br.imgmsg_to_cv2(msg, desired_encoding="passthrough")
            
            # Save the frame to the specified folder
            frame_path = os.path.join(self.image_sequence_dir, f'1000{self.frame_count:04d}.png')
            cv2.imwrite(frame_path, cv_image)
            print(f"Saved frame: {frame_path}")
            
            # Publish RGB image and timestep
            img_msg = self.br.cv2_to_imgmsg(cv_image, encoding="passthrough")
    ...
    def clear_folder(self):
        """Clears the image sequence folder."""
        if os.path.exists(self.image_sequence_dir):
            shutil.rmtree(self.image_sequence_dir)
        os.makedirs(self.image_sequence_dir)
